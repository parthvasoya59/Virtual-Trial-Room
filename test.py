...
def cvloop(run_event):
 ...
 #Necklaces
            if SPRITES[1]:
                ...
                apply_sprite(image, IMAGES[1][ACTIVE_IMAGES[1]],w1,x1,y1+200, incl,  ontop = False)
            
            #Earrings
            (x0,y0,w0,h0) = get_face_boundbox(shape, 6) #bound box of mouth
            if SPRITES[2]:
                (x3,y3,w3,h3) = get_face_boundbox(shape, 7) #nose
                apply_sprite(image, IMAGES[2][ACTIVE_IMAGES[2]],w3,x3-18,y3+80, incl,ontop=False)
                (x3,y3,w3,h3) = get_face_boundbox(shape, 8) #nose
                apply_sprite(image, IMAGES[2][ACTIVE_IMAGES[2]],w3,x3+18,y3+140, incl) 

           #Goggles
            if SPRITES[3]:
                (x3,y3,_,h3) = get_face_boundbox(shape, 1)
                apply_sprite(image, IMAGES[3][ACTIVE_IMAGES[3]],w,x,y3-10, incl, ontop = False)
